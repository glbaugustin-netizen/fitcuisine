...
import { X, ChevronLeft, Clock, Users, Plus, Minus, Dumbbell, Leaf, ShoppingBag, Sun, UtensilsCrossed, Moon, Apple } from 'lucide-react'
import type { Recipe, MealType, TrackerEntry } from '../types'
...
const mealOptions: { type: MealType; label: string; Icon: typeof Sun; color: string }[] = [
  { type: 'breakfast', label: 'Petit-déjeuner', Icon: Sun,              color: '#E8B44A' },
  { type: 'lunch',     label: 'Déjeuner',       Icon: UtensilsCrossed,  color: '#E8713A' },
  { type: 'dinner',    label: 'Dîner',          Icon: Moon,             color: '#8B7EC8' },
  { type: 'snack',     label: 'Snack',          Icon: Apple,            color: '#5AAD2F' },
]
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 rounded-pill h-2" style={{ backgroundColor: '#F0E8E0' }}>
        <div className="h-2 rounded-pill transition-all duration-500 ease-out" style={{ width: `${pct}%`, backgroundColor: color }}
          role="progressbar" aria-valuenow={value} aria-valuemin={0} aria-valuemax={max} />
      </div>
      <span className="font-nunito text-text-secondary w-12 text-right" style={{ fontSize: '12px' }}>{value}g</span>
    </div>
  )
}

export function RecipeDetail({ recipe, onClose, isMobile }: RecipeDetailProps) {
  ...
  const handleAddToMeal = useCallback((mealType: MealType, mealLabel: string) => {
    ...
    addEntry(todayStr(), mealType, entry)
    setShowMealMenu(false)
    setToast(`Ajouté au ${mealLabel} !`)
  }, [recipe, portions, scaled, addEntry])

  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" role="dialog" aria-modal="true" aria-label={`Recette : ${recipe.name}`}>
      ...
      >
        ...
        <div
          ...
          style={{ height: isMobile ? '200px' : '240px', backgroundColor: isMasse ? '#FFF0E6' : '#EEFAE6', borderBottom: '2px solid #E8DDD3' }}
        >
          ...
          <span
            ...
            style={{ backgroundColor: isMasse ? '#D85A30' : '#5AAD2F', color: 'white', borderRadius: '50px', padding: '4px 14px', fontSize: '12px' }}
          >
        ...
        <div className="p-5 md:p-6 space-y-5">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-text-secondary">
              ...
              <span className="font-nunito font-semibold text-sm">{categoryLabel[recipe.category]}</span>
              <span style={{ color: '#E8DDD3' }}>·</span>
              <span className="flex items-center gap-1 font-nunito font-semibold text-sm">
                <Users size={15} strokeWidth={2} aria-hidden="true" />{recipe.servings} portion{recipe.servings > 1 ? 's' : ''} par défaut
              </span>
            </div>
          </div>

          {/* Portions */}
          <div className="flex items-center justify-between rounded-input border-2 border-border-default p-3" style={{ backgroundColor: '#FFF5ED' }}>
            <span className="font-nunito font-semibold text-text-primary text-sm">Nombre de portions</span>
            <div className="flex items-center gap-3">
              <button onClick={decr} disabled={portions <= 1}
                className="w-9 h-9 rounded-full border-2 border-border-strong bg-bg-card flex items-center justify-center transition-all duration-150 hover:bg-bg-section active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
                aria-label="Diminuer les portions">
                <Minus size={16} strokeWidth={2.5} aria-hidden="true" />
              </button>
              <span className="font-fredoka font-semibold text-text-primary w-6 text-center" style={{ fontSize: '20px' }}>{portions}</span>
              <button onClick={incr} disabled={portions >= 10}
                className="w-9 h-9 rounded-full border-2 border-border-strong bg-primary flex items-center justify-center transition-all duration-150 hover:bg-primary-hover active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
          ...
          <div className="rounded-input border-2 border-border-default p-4 space-y-3" style={{ backgroundColor: '#FFF5ED' }}>
            <h3 className="font-fredoka font-semibold text-text-primary" style={{ fontSize: '16px' }}>Valeurs nutritionnelles</h3>
            ...
            <div className="space-y-2">
              <MacroBar label="Protéines" value={scaled.protein} max={Math.max(scaled.protein, scaled.carbs, scaled.fat)} color="#E8713A" />
              <MacroBar label="Glucides"  value={scaled.carbs}   max={Math.max(scaled.protein, scaled.carbs, scaled.fat)} color="#5AAD2F" />
              <MacroBar label="Lipides"   value={scaled.fat}     max={Math.max(scaled.protein, scaled.carbs, scaled.fat)} color="#C46E8E" />
            </div>
          ...
          {/* Ingredients */}
          <div>
            <h3 className="font-fredoka font-semibold text-text-primary mb-3" style={{ fontSize: '18px' }}>Ingrédients</h3>
            <ul className="space-y-2" aria-label="Liste des ingrédients">
              {recipe.ingredients.map((ing, i) => {
                const qty = Math.round(ing.quantity * ratio * 10) / 10
                return (
                  <li key={i} className="flex items-center justify-between py-2 font-nunito" style={{ borderBottom: '1px solid #E8DDD3' }}>
                    <span className="text-text-primary font-semibold text-sm">{ing.name}</span>
                    <span className="font-nunito font-bold text-sm rounded-macro px-2 py-0.5"
          ...
          {/* Steps */}
          <div>
            <h3 className="font-fredoka font-semibold text-text-primary mb-3" style={{ fontSize: '18px' }}>Préparation</h3>
          ...
          <div className="relative">
            <button
              ...
              style={{ backgroundColor: '#E8713A', borderRadius: '50px', padding: '14px 24px' }}
            >
            ...
            {showMealMenu && (
              <div
                className="absolute bottom-full left-0 right-0 mb-2 rounded-card border-2 border-border-strong shadow-modal z-10"
                style={{ backgroundColor: 'white' }}
                role="menu"
                aria-label="Choisir un repas"
              >
                <p className="font-nunito font-semibold text-text-secondary uppercase px-4 pt-3 pb-2" style={{ fontSize: '11px', letterSpacing: '0.5px' }}>
                  Ajouter à quel repas ?
                </p>
                {mealOptions.map(({ type, label, Icon, color }) => (
                  <button
                    key={type}
                    onClick={() => handleAddToMeal(type, label)}
                    className="w-full flex items-center gap-3 px-4 py-3 hover:bg-bg-section transition-colors last:rounded-b-card"
                    role="menuitem"
                  >
                    <div className="w-8 h-8 rounded-full border-2 border-border-strong flex items-center justify-center flex-shrink-0"
                      style={{ backgroundColor: color }}>
                      <Icon size={14} strokeWidth={2} className="text-white" aria-hidden="true" />
                    </div>
                    <span className="font-nunito font-semibold text-text-primary text-sm">{label}</span>
                  </button>
                ))}
              </div>
            )}
